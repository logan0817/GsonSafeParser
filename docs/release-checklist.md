...
发布 `1.0.0` 前按这份清单核对，目标是确认 Android AAR、consumer ProGuard、release 混淆自检、文档版本号和 Maven 本地产物都稳定。
...
9. 构建过程中不能出现 Gradle warning、Kotlin warning 或配置期 classpath 解析告警。

## 2. AAR 产物检查

本地 Maven 产物由 `verifyMavenLocalPublicationArtifacts` 统一校验，必须满足：
1. `gson-safe-parser-core-1.0.0.aar` 和 `gson-safe-parser-retrofit-1.0.0.aar` 存在。
2. 主产物不能退回普通 `.jar`。
...
发布前必须确认：
1. 根 `build.gradle.kts` 版本是 `1.0.0`。
2. demo `versionName` 是 `1.0.0`。
3. `README.md`、`README_EN.md`、`docs/getting-started.md`、`docs/en/getting-started.md` 同时包含 core 和 retrofit 的 `1.0.0` 坐标。
4. 中文文档和英文文档入口互相可跳转。
5. README 文档表能找到快速开始、兼容性说明、配置说明、错形能力矩阵（JSON 形状不一致）、Android 混淆、Demo App、排障指南、发布清单和 1.0.0 发布说明。
...
7. `CHANGELOG.md` 存在，并以 `1.0.0` 作为首个公开发布版本。
8. `docs/release-notes-1.0.0.md` 和 `docs/en/release-notes-1.0.0.md` 明确列出首发能力、兼容边界和发布验证。
