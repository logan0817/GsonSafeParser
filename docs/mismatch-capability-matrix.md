...
| 对象和数组形态转换 | 默认关闭。 | 只在显式开启后处理字段级对象/数组漂移。 |
...
1. 默认状态：`ShapeCoercionPolicy.Disabled`，不做对象和数组互转，保持旧版本默认行为。
2. 对象字段：`data: User` 收到 `"data":[{"id":1}]` 时，显式开启 `ObjectFromFirstArrayItem` 后读取数组第 1 个对象。
3. 集合字段：`users: List<User>` 或 `users: Set<User>` 收到 `"users":{"id":1}` 时，显式开启 `CollectionFromSingleObject` 后包装成 1 个元素的集合。
4. 对象数组字段：`users: Array<User>` 收到 `"users":{"id":1}` 时，显式开启 `CollectionFromSingleObject` 后包装成长度为 1 的数组。
5. 观测证据：`ShapeCoercion`，报告包含 `path`、`fieldName`、`shapeCoercionAction` 和 `discardedItemCount`。
6. 边界：根级对象、根级集合、根级对象数组、Map、数字、布尔值、字符串不会被转换，也不会把字符串内容当 JSON 再解析。
7. 失败处理：空数组、数组第 1 项不是对象、转换时 Adapter 失败，会记录 `ShapeCoercion` 事件并回到原兜底行为。
8. 不可安全隔离问题：`Error`、`ThreadDeath`、`LinkageError`、`CancellationException` 和真实传输 I/O 继续外抛，不会变成转换失败事件。
...
3. 观测证据：`TypeMismatch` 和字段 path；Map item key 默认省略，显式设置 `MapItemKeyPolicy.Hash` 时才会带 `sha256:` 哈希后的 `mapItemKey`。
