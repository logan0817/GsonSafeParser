...
Run this checklist before publishing `1.0.0`. The goal is to verify Android AAR artifacts, consumer ProGuard rules, release minification checks, documentation versions, and local Maven artifacts.
...
1. `gson-safe-parser-core-1.0.0.aar` and `gson-safe-parser-retrofit-1.0.0.aar` exist.
...
Before publishing, confirm:
1. root `build.gradle.kts` version is `1.0.0`.
2. demo `versionName` is `1.0.0`.
3. `README.md`, `README_EN.md`, `docs/getting-started.md`, and `docs/en/getting-started.md` contain both core and retrofit `1.0.0` coordinates.
4. Chinese and English docs can link to each other.
5. The README documentation table links to Getting Started, Compatibility, Configuration, Mismatch Capability Matrix, Android ProGuard, Demo App, Troubleshooting, Release Checklist, and 1.0.0 Release Notes.
...
7. `CHANGELOG.md` exists and treats `1.0.0` as the first public release.
8. `docs/release-notes-1.0.0.md` and `docs/en/release-notes-1.0.0.md` list initial capabilities, compatibility boundaries, and release verification.
