...
| Library | `1.0.0` | First public release | Internal pre-1.0 iterations are not treated as the public compatibility baseline. | Use `1.0.0` for new integration. |
...
If your project already uses a newer Retrofit version, Gradle can usually resolve to the newer version. Before production, verify 3 things:
