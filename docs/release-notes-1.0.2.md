...
4. 普通自定义 Adapter 抛出的 `IOException` 在 `1.0.2` 当时仍保持局部兜底行为；后续版本已逐步收紧，`1.0.4` 继续保留原生 Gson 自定义 Adapter 链路，异常向外抛出。
...
如果 App 断网、请求取消或 OkHttp 流被重置，`1.0.2` 会让这些异常按网络层错误继续向外传播。
